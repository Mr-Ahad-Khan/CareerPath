...
export function MilestonesPage() {
  ...
  const addMilestone = async () => {
    ...
    try {
      // Ensure description is never an empty string (Mongoose required validator treats "" as empty)
      const payload = {
        ...newMs,
        description: newMs.description.trim() || newMs.title.trim(),
      };
  ...
  const byColumn = (key) =>
    milestones
      ...
      .sort((a, b) => a.year - b.year || a.orderIndex - b.orderIndex);
    ...
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex gap-2">
          <button onClick={logActivity} className="btn-secondary">
            <Flame className="h-4 w-4 text-accent" /> Log today's practice
          </button>
            ...
            <div key={col.key} className="surface-card p-4">
              ...
              <div className="space-y-2.5">
                {byColumn(col.key).map((m) => (
                  <div
                    key={m.id}
                    className="group rounded-xl border border-border bg-surface-2 p-3 transition-all hover:border-accent/30"
                  >
      ...
      {showAdd && (
        <div
          className="fixed inset-0 z-[80] flex items-center justify-center bg-background/80 p-4 backdrop-blur-sm animate-fade-in-flat"
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  value={newMs.title}
                  onChange={(e) =>
                    setNewMs({ ...newMs, title: e.target.value })
                  }
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  ...
                  <select
                    ...
                    value={newMs.year}
                    onChange={(e) =>
                      setNewMs({ ...newMs, year: +e.target.value })
                    }
                  >
                    {[0, 1, 2, 3, 4, 5].map((y) => (
                ...
                <div>
                  ...
                  <select
                    ...
                    value={newMs.quarter}
                    onChange={(e) =>
                      setNewMs({ ...newMs, quarter: e.target.value })
                    }
                  >
                    {["Q1", "Q2", "Q3", "Q4"].map((q) => (
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setNewMs({ ...newMs, category: e.target.value })
                    }
                  >
                    {[
                      "Learning",
              ...
              <button onClick={addMilestone} className="btn-primary w-full">
                Add milestone
              </button>
            </div>
          </div>
