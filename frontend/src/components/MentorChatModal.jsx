...
export function MentorChatModal({ isOpen, connection, onClose, onMessageSent }) {
  ...
  return (
    <div
      className="fixed inset-0 z-[90] flex items-center justify-center bg-background/80 p-0 sm:p-4 backdrop-blur-md animate-fade-in-flat"
      onClick={onClose}
    >
      <div
        className="flex h-[100dvh] sm:h-[88vh] sm:max-h-[680px] w-full max-w-2xl flex-col overflow-hidden rounded-none sm:rounded-2xl border-0 sm:border border-border bg-surface shadow-lift"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between border-b border-border bg-surface-2/70 px-4 sm:px-5 py-3 sm:py-3.5 backdrop-blur-sm shrink-0">
          <div className="flex items-center gap-3 min-w-0">
            <div className="relative">
              <Avatar name={partnerName} color={partner.avatarColor} size={40} />
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-3.5 touch-scroll min-h-0">
          {loading && messages.length === 0 ? (
            ...
            <div className="flex h-full flex-col items-center justify-center text-center p-4 sm:p-6 space-y-3">
                    ...
                    <div className="flex items-end gap-2 max-w-[88%] sm:max-w-[75%]">
                      ...
                      <div
                        className={`px-4 py-2.5 text-sm shadow-sm ${
                          isMyMessage
                            ? 'bg-accent text-accent-contrast rounded-2xl rounded-br-xs font-normal'
                            : 'bg-surface-2 text-foreground border border-border rounded-2xl rounded-bl-xs'
                        }`}
                      >
        ...
        {messages.length > 0 && (
          <div className="border-t border-border/60 bg-surface-2/40 px-3 sm:px-4 py-2 overflow-x-auto flex gap-2 no-scrollbar shrink-0">
            {SUGGESTED_PROMPTS.slice(0, 3).map((prompt) => (
              <button
                key={prompt}
                onClick={() => handleSend(prompt)}
                className="chip shrink-0 text-[11px] py-1 px-2.5 hover:border-accent/50 hover:text-accent transition-colors truncate max-w-[240px]"
              >
                {prompt}
              </button>
            ))}
        ...
        {/* Input Bar */}
        <div className="border-t border-border bg-surface p-3 sm:p-4 shrink-0">
          <div className="relative flex items-center gap-2">
            <input
              ...
              type="text"
              className="field-input flex-1 text-sm rounded-xl"
            ...
            <button
              ...
              className="btn-primary shrink-0 px-3.5 py-2.5 rounded-xl disabled:opacity-40 disabled:cursor-not-allowed"
